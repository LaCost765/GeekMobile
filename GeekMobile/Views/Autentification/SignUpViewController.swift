...
class SignUpViewController: UIScrollableViewController {
    ...
    var viewModel: SignUpViewModel = SignUpViewModel(model: SignUpModel())
    ...
    @IBAction func signUpButtonTapped(_ sender: Any) {
        viewModel.signUp { success, message in
            if success {
                ...
                self.showNotificationAlert(title: "Ошибка регистрации!",
    ...
    func bindViewModel() {
        ...
        nameView.textField.rx.text.bind(to: viewModel.model.name).disposed(by: viewModel.bag)
        surnameView.textField.rx.text.bind(to: viewModel.model.surname).disposed(by: viewModel.bag)
